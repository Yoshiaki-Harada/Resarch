...
object CostMinProviderAuction : LpMaker {
    ...
    override fun makeLpFile(config: Config, obj: Object, bidders: List<Bidder>, vararg option: Option) {
        ...
        //制約条件
        lp.subto()
        writeSubToProvide(lp, obj, providers, requesters, config)
        writeSubToRequest(lp, obj, providers, requesters, config)
        writeSubToWinner(lp, obj, providers, requesters, config)
    ...
    fun writeObjFunction(lp: LpWriter, obj: lpfile.lpformat.Object, providers: List<Bidder>, requesters: List<Bidder>, config: Config) {
        lp.obj(obj)
    ...
    //提供時間制約
    fun writeSubToProvide(lp: LpWriter, obj: lpfile.lpformat.Object, providers: List<Bidder>, requesters: List<Bidder>, config: Config) {
        //全てのresouceについて
        providers.forEachIndexed { i, provider ->
            provider.bids.forEachIndexed { r, resource ->
    ...
    //要求制約
    fun writeSubToRequest(lp: LpWriter, obj: lpfile.lpformat.Object, providers: List<Bidder>, requesters: List<Bidder>, config: Config) {
        requesters.forEachIndexed { j, requester ->
            requester.bids.forEachIndexed { n, bid ->
    ...
    //勝者となれるのは1企業のみ
    fun writeSubToWinner(lp: LpWriter, obj: lpfile.lpformat.Object, providers: List<Bidder>, requesters: List<Bidder>, config: Config) {
        requesters.forEachIndexed { j, requester ->
            requester.bids.forEachIndexed { n, bid ->
                bid.bundle.forEachIndexed { r, time ->
