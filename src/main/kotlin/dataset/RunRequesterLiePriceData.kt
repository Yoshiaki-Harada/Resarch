...
//     提供割合の違いの繰り返し
    for (s in 0 until SUPPLY_DATASET_NUMBER) {
        ...
        val bidDir = "Bid/Provider=25/budget-range/lie-ratio/2.5/0%"
        ...
        for (p in 0 until 3) {
            ...
            for (i in 0 until DATASET_ITERATE) {
                ...
                val copyBidders = bidders
                        ...
                        .mapIndexed { bidderIndex, bidder ->
                            if (bidderIndex < 1) Bidder().add(
                                    bidder.bids.map {
                                        println(lieMap["$lie%"]!!.times(it.value.tValue).times(-1.0))
                                        Bid(Value(it.value.tValue, lieMap["$lie%"]!!.times(it.value.tValue).times(-1.0)), it.bundle)
                                    }
                            ) else bidder
                        }

                val afterBidders = bidders.subList(0, config.provider).plus(copyBidders)
                val dir = File("Bid/Provider=25/budget-range/lie-ratio/2.5/$lie%/$i").absoluteFile
                dir.mkdirs()
